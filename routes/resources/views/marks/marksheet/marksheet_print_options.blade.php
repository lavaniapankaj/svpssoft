            ...
            <div class="col-md-12">
                <div class="card border-0 bg-white">
                    <div class="card-header flex-wrap bg-white d-flex align-items-center justify-content-between">
                        <h5 class="mb-0 mt-0">{{ 'Select Exams' }}</h5>
                    ...
                    <div class="card-body">
                        <form id="submitForm" action="{{ route('marks.marks-report.select.exam.store') }}" method="POST">
                            @csrf
                                    ...
                                    @if ($data)
                                        @foreach ($data as $key => $value)
                                            <tr data-entry-id="{{ $key }}" id="exam-row">
                                                <td>{{ $value }}</td>
                                                <td><input type="checkbox" id="without" name="without"value={{ $key }}></td>
                                                <td><input type="checkbox" id="with" name="with" value={{ $key }}></td>
                                            </tr>
                                        @endforeach
                                    @else
                                        <tr>
