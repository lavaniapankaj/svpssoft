...
<div class="container-fluid">
    ...
    <div class="row ">
        <div class="col-md-12">
           ...
           <div class="card-header flex-wrap bg-white d-flex align-items-center justify-content-between"><h5 class="mb-0 mt-0">{{ 'Edit Student Marks' }}</h5>
                ...
                <div class="card-body">
                    <form method="POST" action="" id="update-form">
                        ...
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label for="marks" class="mt-2">Enter Marks <span class="text-danger">*</span></label>
                                <input type="text" name="marks" id="marks" class="form-control @error('marks') is-invalid @enderror">
                        ...
                        {{-- <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                <label class="form-check-label mt-4" for="flexCheckDefault">Attendance(Check for Present) <span class="text-danger">*</span></label>
                                <input type="checkbox" name="attendance" id="flexCheckDefault" value="1" class="form-check-input mt-4 border border-black @error('attendance') is-invalid @enderror">

                                <span class="invalid-feedback form-invalid fw-bold attendance-error" role="alert"></span>

                            </div>
                        </div> --}}
                        <div class="row">
                            <div class="form-group col-md-6">
                                <label class="mt-4">Attendance <span class="text-danger">*</span></label><br>
                                <div class="form-check form-check-inline">
                                    <input type="checkbox" name="attendance" id="presentCheck" value="1"
                                        class="form-check-input @error('attendance') is-invalid @enderror">
                                    <label class="form-check-label" for="presentCheck">Present</label>
                                </div>
                                <div class="form-check form-check-inline">
                                    <input type="checkbox" name="attendance" id="absentCheck" value="0" class="form-check-input @error('attendance') is-invalid @enderror">
                        ...
                        <div class="mt-3">
                            ...
                            <span><img src="{{ config('myconfig.myloader') }}" alt="Loading..." class="loader" id="loader" style="display:none; width:10%;"></span>
                        </div>
...
<script>
    $(document).ready(function() {
        ...
        // Mutually exclusive Present/Absent
        $('#presentCheck').on('change', function() {
            if (this.checked) {
                $('#absentCheck').prop('checked', false);
            }
        });
        $('#absentCheck').on('change', function() {
            if (this.checked) {
                $('#presentCheck').prop('checked', false);
                $('#marks').val(''); // clear marks if absent
            }
        });

        $('#marks-updateBtn').on('click', function() {
            let marks = $('#marks').val().trim();
            let isPresent = $('#presentCheck').is(':checked');
            let isAbsent = $('#absentCheck').is(':checked');
            ...
            if (marks != '' && !isPresent) {
                $('.attendance-error').show().html('Present checkbox is required when marks are entered.');
                return;
            }
            if (marks == '' && !isAbsent) {
                $('.attendance-error').show().html('Absent checkbox is required when no marks are entered.');
                return;
            }
            $.ajax({
                ...
                success: function(data) {
                    if (data.status == 'success') {
                        ...
                        Swal.fire({
                            ...
                            // location.reload();
                            $('#marks').val('');                // clear marks input
                            $('#flexCheckDefault').prop('checked', false); // uncheck attendance checkbox
                        });
                    } else {
                        Swal.fire({
                            title: 'Error',
