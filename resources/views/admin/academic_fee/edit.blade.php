            ...
            <div class="col-md-12">
                <div class="card border-0 bg-white">
                    <div class="card-header flex-wrap bg-white d-flex align-items-center justify-content-between">
                        <h5 class="mb-0 mt-0">{{ 'Edit Academic Fee' }}</h5>
                            <a href="{{ route('admin.academic-fee-master.index') }}" class="btn bg-light btn-sm" ><span class="mdi mdi-chevron-left me-2"></span>Back</a>

                        </div>

                        <div class="card-body">
                            <form action="{{ route('admin.academic-fee-master.update', $feeMaster->id ?? '') }}" method="POST" id="basic-form">
                                @csrf
                                @method('put')
                                <input type="hidden" name="id" id="id"
                                    value="{{ isset($feeMaster) ? $feeMaster->id : '' }}">

                                <div class="row">
                                    <div class="form-group col-md-6">
                                        <label for="session_id" class="mt-2"> Session <span
                                                class="text-danger">*</span></label>
                                        <!-- <input type="hidden" id="initialSectionId" value="{{ $feeMaster->session_id ?? '' }}"> -->
                                        <select name="session_id" id="session_id"
                                            ...
                                            @if (count($sessions) > 0)
                                                @foreach ($sessions as $key => $session)
                                                    <option value="{{ $key }}"
                                                        {{ isset($feeMaster) && $feeMaster->session_id == $key ? 'selected' : '' }}>
                                                        {{ $session }}</option>
                                                @endforeach
                                            @else
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <!-- <input type="hidden" id="initialClassId"
                                            value="{{ isset($feeMaster) ? $feeMaster->class_id : '' }}"> -->
                                        <select name="class_id" id="class_id"
                                            ...
                                            @if (count($classes) > 0)
                                                @foreach ($classes as $key => $class)
                                                    <option value="{{ $key }}"
                                                        {{ isset($feeMaster) && $feeMaster->class_id == $key ? 'selected' : '' }}>
                                                        {{ $class }}</option>
                                                @endforeach
                                            @else
                                    ...
                                    <div class="row mt-1">
                                        <div class="form-group col-md-6 mt-2">
                                            ...
                                            <input type="text" name="admission_fee" id="admission_fee"
                                                ...
                                                value="{{ old('admission_fee', isset($feeMaster) ? $feeMaster->admission_fee : '') }}"
                                                required>
                                            @error('admission_fee')
                                        ...
                                        <div class="form-group col-md-6 mt-2">
                                            ...
                                            <input type="text" name="inst_1" id="inst_1"
                                                ...
                                                value="{{ old('inst_1', isset($feeMaster) ? $feeMaster->inst_1 : '') }}" required>
                                            @error('inst_1')
                                                <span class="invalid-feedback form-invalid fw-bold"
                                                    role="alert">{{ $message }}</span>
                                            @enderror
                                        ...
                                        <div class="form-group col-md-6 mt-2">
                                            ...
                                            <input type="text" name="inst_2" id="inst_2"
                                                ...
                                                value="{{ old('inst_2', isset($feeMaster) ? $feeMaster->inst_2 : '') }}" required>
                                            @error('inst_2')
                                                <span class="invalid-feedback form-invalid fw-bold"
                                                    role="alert">{{ $message }}</span>
                                            @enderror
                                        ...
                                        <div class="form-group col-md-6 mt-2">
                                            ...
                                            <input type="text" name="ins_discount" id="ins_discount"
                                                ...
                                                value="{{ old('ins_discount', isset($feeMaster) ? $feeMaster->ins_discount : '') }}"
                                                required>
                                            @error('ins_discount')
                                                <span class="invalid-feedback form-invalid fw-bold"
                                                    role="alert">{{ $message }}</span>
                                        ...
                                        <div class="form-group col-md-12 mt-2">
                                            ...
                                            <input type="text" name="inst_total" id="inst_total"
                                                ...
                                                value="{{ old('inst_total', isset($feeMaster) ? $feeMaster->inst_total : '') }}"
                                                readonly>
                                            @error('inst_total')
                                                <span class="invalid-feedback form-invalid fw-bold"
                                                    role="alert">{{ $message }}</span>
