        ...
        <div class="col-md-12">
            <div class="card border-0 bg-white">
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.overAllGrade.store') }}" method="POST" id="overallGradeForm">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select name="exam_id" id="examSelect" class="form-control @error('exam_id') is-invalid @enderror" required>
                                ...
                                @foreach ($exams as $key => $exam)
                                    <option value="{{ $key }}" {{ old('exam_id', $selectedExam ?? '') == $key ? 'selected' : '' }}>
                                        {{ $exam }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="grades-container mb-3 {{ old('grade_type') == 'overall' && old('exam_id') && old('class_id') ? '' : 'd-none' }}" id="overallGradesSection">
                        ...
                        <div class="grades-container mb-3 {{ old('grade_type') == 'subject_wise' && old('exam_id') && old('class_id') ? '' : 'd-none' }}" id="subjectWiseGradesSection">
                            ...
                            <div id="subjectWiseContainer">
                                ...
                                @else
                                    <p class="text-center text-muted">Select exam and class to load subjects</p>
                                @endif
                            </div>
                        </div>

                        {{-- Submit --}}
                        <div class="mt-4 {{ (old('grade_type') && old('exam_id') && old('class_id')) ? '' : 'd-none' }}" id="saveSection">
...
$(document).ready(function () {
    ...
    let hasOldInput = {{ old('grade_type') && old('exam_id') && old('class_id') ? 'true' : 'false' }};
    ...
    if (hasOldInput) {
        ...
        if (gradeType === 'subject_wise' && examId && classId) {
            // Load subject-wise data on validation error
            loadSubjectWiseGrades(examId, classId, true);
        }
    }
    ...
    $('#gradeTypeSelect').change(function () {
        ...
        // Check if exam and class are selected
        const examId = $('#examSelect').val();
        const classId = $('#classSelect').val();

        if (examId && classId) {
            loadGradesByType(gradeType, examId, classId);
        }
    });

    // Load grades when exam & class selected
    $('#examSelect, #classSelect').change(function () {
        const examId = $('#examSelect').val();
        const classId = $('#classSelect').val();
        const gradeType = $('#gradeTypeSelect').val();

        // Hide sections if any required field is not selected
        if (!examId || !classId || !gradeType) {
            $('#overallGradesSection').addClass('d-none');
            $('#subjectWiseGradesSection').addClass('d-none');
            $('#saveSection').addClass('d-none');
            return;
    ...
    function loadGradesByType(gradeType, examId, classId) {
        if (gradeType === 'overall') {
            loadOverallGrades(examId, classId);
        } else if (gradeType === 'subject_wise') {
            loadSubjectWiseGrades(examId, classId, false);
        }
    }

    function loadOverallGrades(examId, classId) {
        ...
        $.ajax({
            ...
            type: "GET",
            data: { exam: examId, class: classId },
    ...
    function loadSubjectWiseGrades(examId, classId, skipLoad = false) {
        ...
        $.ajax({
            url: "{{ route('admin.subjectWiseOverallGrade.get') }}",
            type: "GET",
            data: { exam: examId, class: classId },
            success: function (res) {
