                ...
                <div class="card border-0 bg-white">
                    <div class="card-header bg-white">
                        <div class="row align-items-center">
                            <div class="col-md-4">
                                <h5 class="mb-0">{{ __('Subject Master') }}</h5>
                            </div>
                            <div class="col-md-8">
                                <div class="d-flex align-items-center justify-content-end gap-2">
                                    <!-- Filter Form -->
                                    <form action="{{ route('admin.subject-master.index') }}" method="get"
                                        class="d-flex align-items-center gap-2">
                                        <select name="class_id" id="filter_class_id" class="form-select" style="width: 180px;">
                                        ...
                                        @if (request()->get('class_id'))
                                            <a href="{{ route('admin.subject-master.index') }}" class="btn btn-dark" title="Reset Filter">
                                                Reset
                                            </a>
                                        @endif
                                    </form>
                    ...
                    <div class="card-body border-bottom bg-light" id="createFormSection" style="display: none;">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            <div>
                                <h6 class="mb-1 fw-bold">Add New Subject</h6>
                                <small class="text-muted">Fill in the details below to create a new subject</small>
                            </div>
                        </div>

                        <form action="{{ route('admin.subject-master.store') }}" method="POST" id="inline-create-form">
                            @csrf
                            <div class="row g-3">
                                <!-- Class Selection -->
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="text" name="subject" id="subject" class="form-control @error('subject') is-invalid @enderror" placeholder="e.g., Mathematics, English" value="{{ old('subject') }}" required autocomplete="off">
                                    @error('subject')
                                        <div class="invalid-feedback">
                                            {{ $message }}
                                        </div>
                                ...
                                <!-- Result Type -->
                                <div class="col-md-4">
                                    <label class="form-label fw-semibold"> Result Type <span class="text-danger">*</span>
                                    </label>
                                    <div class="d-flex gap-3 mt-2">
                                        <div class="form-check">
                                            <input class="form-check-input @error('by_m_g') is-invalid @enderror" type="radio" name="by_m_g" id="by_marks" value="1" {{ old('by_m_g') == 1 ? 'checked' : '' }}>
                                            <label class="form-check-label" for="by_marks">
                                                By Marks
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input class="form-check-input @error('by_m_g') is-invalid @enderror" type="radio" name="by_m_g" id="by_grade" value="2" {{ old('by_m_g') == 2 ? 'checked' : '' }}>
                                            ...
                                            <tr data-entry-id="{{ $value->id }}">
                                                ...
                                                <td class="text-center">
                                                    <div class="d-flex gap-2 justify-content-center">
                                                        <a href="{{ route('admin.subject-master.edit', $value->id) }}"
                                    ...
                                    @else
                                        <tr>
                                            <td colspan="4" class="text-center text-muted py-4">
                                                <i class="mdi mdi-information-outline me-2"></i>No Subjects Found
                                            </td>
                                        </tr>
                                    @endif
                                </tbody>
    ...
    <script>
        ...
        document.getElementById('toggleFormBtn').addEventListener('click', function() {
            ...
            if (formSection.style.display === 'none') {
                ...
                btn.innerHTML = '<i class="mdi mdi-plus-circle-outline me-2"></i>Add New Subject';
                ...
                document.getElementById('by_marks').checked = false;
                document.getElementById('by_grade').checked = false;
        ...
        document.querySelectorAll('input[name="by_m_g"]').forEach(radio => {
            radio.addEventListener('change', function() {
                document.querySelectorAll('input[name="by_m_g"]').forEach(r => {
                    r.classList.remove('is-invalid');
                });
                const feedback = this.closest('.col-md-4').querySelector('.invalid-feedback');
                if (feedback) {
